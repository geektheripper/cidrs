export class IpRange {
  ...
  intersectionTest(v: IpRange): [intersected: boolean, touched: boolean, start: number, end: number] {
  ...
  enroll(v: IpRange) {
    const [intersected, touched] = this.intersectionTest(v);
    if (!intersected && !touched) throw "cant't merge, not intersected or touched"
...
export class IpRanges {
  ...
  // constructor() { }
  setBoundary(boundary: IpRange) { this.boundary = boundary; return this }
  insert(interval: IpRange) {
    if (this.boundary && !this.boundary.includes(interval)) throw "ip interval out of boundary"
    ...
    for (let i = 0; i < this.ranges.length; i++) {
      ...
      if (currTouched) {
        ...
        if (next) {
          const [nextIntersected, nextTouched] = interval.intersectionTest(next)
