...
export class IP {
  ...
  constructor(address: string | number) {
    if (typeof address === "number") {
      if (address < 0 && address >= 2 ** 32)
        throw `ip out of range: ${address}`;
      this.value = address;
      this.array = [
        (address >> 24) & 255,
        (address >> 16) & 255,
    ...
    const [, ...parts] = result.map((i) => Number(i)) as [
      number,
      number,
      number,
      number,
    ...
    for (const p of parts)
      if (isNaN(p) || p < 0 || p > 255) throw `illegal ip: ${address}`;
    this.raw = address;
    this.array = parts;
    this.value =
...
export class CIDR {
  ...
  readonly networkAddress = new IP(
    Math.floor(this.address.value / this.addressNum) * this.addressNum
  );
  readonly broadcastAddress = new IP(
    this.networkAddress.value + this.addressNum - 1
  );
  ...
  readonly firstAvailableAddress =
    this.prefixLength > 30 ? null : this.networkAddress.next();
  readonly lastAvailableAddress =
    this.prefixLength > 30 ? null : this.broadcastAddress.prev();
  readonly hostRange =
    this.prefixLength > 30
      ? null
      : ([this.firstAvailableAddress, this.lastAvailableAddress] as [IP, IP]);
  #maskIP = new IP(2 ** 32 - this.addressNum);
  readonly mask = this.#maskIP.raw;

  #range = new IpRange(
    this.networkAddress.value,
    this.broadcastAddress.value + 1
  );
  #pool = new IpRanges().setBoundary(this.#range);

  static fromString(cidr: string) {
    const result = cidr.match(
    ...
    if (isNaN(prefixLength) || prefixLength < 0 || prefixLength > 32)
      throw `illegal prefix: ${cidr}`;

    return new CIDR(new IP(ip as string), prefixLength);
  }
  toString() {
    return `${this.networkAddress.raw}/${this.prefixLength}`;
  }
  ...
  mkSubnets(prefixIncrement: number, start = 0, end?: number) {
    ...
    if (prefixIncrement + this.prefixLength >= 32)
      throw `illegal prefix length: ${subnetPrefixLength}`;
    ...
    if (start >= subnetNum || start < 0)
      throw `subnet start index out of range: ${start}`;
    if (end > subnetNum || end < 0)
      throw `subnet end index out of range: ${start}`;
  ...
  allocateSubnets(prefixIncrement: number, start = 0, end?: number) {
    ...
    for (const sn of subnets) {
      try {
        this.#pool.insert(sn.#range);
      } catch {
        throw `subnet already allocated: ${sn.toString()}`;
      }
    }
    return subnets;
  }
  take(cidr: CIDR) {
    this.#pool.insert(cidr.#range);
    return cidr;
  }
  fromRange(range: IpRange) {
    const prefixLength = 32 - Math.log2(range.length);
    if (!Number.isInteger(prefixLength))
      throw `illegal network capacity: ${range.length}`;

    const start = new IP(range.start);
    const end = new IP(range.end);
    const cidr = new CIDR(start, prefixLength);
    if (
      cidr.networkAddress.value === range.start &&
      cidr.broadcastAddress.value === range.end - 1
    )
      return cidr;
